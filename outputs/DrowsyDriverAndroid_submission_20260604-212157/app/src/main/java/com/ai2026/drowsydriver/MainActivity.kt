...
class MainActivity : ComponentActivity() {
    ...
    private fun onLandmarkerResult(result: FaceLandmarkerResult, input: com.google.mediapipe.framework.image.MPImage) {
        ...
        val status = heuristicStatus.copy(
        ...
        runOnUiThread {
            ...
            if (status.state != DriverState.NO_FACE) {
                val cnnState = if (tfliteClassifier?.isAvailable == true) "CNN ready" else "CNN model pending"
    ...
    private fun classifyEyeRoi(result: FaceLandmarkerResult): CnnPrediction? {
        ...
        val eyeCrop = cropEyeBand(bitmap, face) ?: return null
        return classifier.predict(eyeCrop)
    }

    private fun cropEyeBand(bitmap: Bitmap, face: List<NormalizedLandmark>): Bitmap? {
        val eyeIndices = intArrayOf(
        ...
        val marginX = (width * 0.35f).toInt()
        val marginY = (height * 1.40f).toInt()
        val rect = Rect(
            max(0, minX - marginX),
            max(0, minY - marginY),
